...
namespace VIVOSHOP.Controllers
{
    public class HomeController : Controller
    {
        ...
        public ActionResult Index(string keyword)
        {
            ...
            {
                //var val = db.Products.Where(x => x.Pro_Name.Contains(keyword)).ToList();
                //if (val.Count() > 0)
                //{
                //    return View(db.Products.Where(x => x.Pro_Name.Contains(keyword)).ToList());
                //}
                return View(db.Products.Where(x => x.Pro_Name.Contains(keyword) || x.ProductType.ProType_Name.Contains(keyword)).ToList());
                //else
                //{
                //    return View(db.Products.OrderBy(x => x.ProductType.ProType_Name.Contains(keyword)).ToList());
                //}
            }
